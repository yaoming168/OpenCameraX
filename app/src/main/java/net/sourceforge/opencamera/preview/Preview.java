...
/*
* 这个类最初的命名是由于封装了摄像头预览，但在实践中，它会变得更大，包括大部分相机的操作。
* 它存在于比摄像机控制器更高的层次上(也就是说，这不仅仅是一个低级的相机API包装，而是支持大部分开放相机的逻辑和功能)。
* 通信应用程序的其余部分可以通过ApplicationInterface获得。我们可以将这个类解耦为单独的组件!
*/
...
//    惰性创建，所以我们不会占用资源，如果应用程序没有使用渲染脚本
...
//    最后一次更新预览的时间
    ...
    private boolean want_histogram; // whether to generate a histogram（直方图）, requires want_preview_bitmap==true
    ...
    public volatile boolean test_ticker_called; // set from CameraXSurfaceView or CanvasView
    ...
    public Preview(ApplicationInterface applicationInterface, ViewGroup parent) {
        ...
        if( using_texture_view ) {
            this.cameraSurface = new CameraXTextureView(getContext(), this);
            ...
            this.cameraSurface = new CameraXSurfaceView(getContext(), this);
    ...
    public void stopVideo(boolean from_restart) {
        ...
        if( video_recorder == null ) {
            ...
            // (important to exit, otherwise we'll momentarily switch the take photo icon to video mode in CameraXApplicationInterface.stoppingVideo() when opening the settings in landscape mode
            if( CameraXDebug.LOG )
    ...
    public void setupCamera(boolean take_photo) {
        ...
        if( camera_controller.isBurstOrExpo() ) {
            ...
            if( CameraXDebug.LOG && current_size != null ) {
    ...
    private void setupCameraParameters() {
...

        {
            ...
            if( this.using_face_detection ) {
                class CameraXFaceDetectionListener implements CameraController.FaceDetectionListener {
                ...
                camera_controller.setFaceDetectionListener(new CameraXFaceDetectionListener());
            }
        }
        if( CameraXDebug.LOG ) {
    ...
    private void onVideoInfo(int what, int extra) {
        ...
        if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_FILESIZE_APPROACHING && video_restart_on_max_filesize ) {
            ...
            if( video_recorder == null ) {
                ...
                // set in CameraXApplicationInterface.getVideoMaxFileSizePref() due to the remaining disk space.
    ...
    private static class RefreshPreviewBitmapTask extends AsyncTask<Void, Void, RefreshPreviewBitmapTaskResult> {
        ...
        protected void onPostExecute(RefreshPreviewBitmapTaskResult result) {
			...
			/*if( CameraXDebug.LOG && preview.histogram != null ) {
